...
void enter_callback(GSimpleAction *action, GVariant *parameter, gpointer data) {
	...
	gtk_widget_override_font(wid->greeterlabel, pango_font_description_from_string("Tahoma 20"));
...
static void activate(GtkApplication *app, gpointer data) {
	...
	const GActionEntry app_actions[] = { { "enter", enter_callback, NULL, NULL, NULL }, { "clear", clear_callback, NULL, NULL, NULL } };
	...
	gtk_entry_set_placeholder_text(GTK_ENTRY(wid->nameentry), "**");
	gtk_grid_attach(GTK_GRID(grid), wid->nameentry, 1, 1, 1, 1);
	g_signal_connect(G_OBJECT(wid->nameentry), "activate", G_CALLBACK (entry_callback), (gpointer )wid);
	...
	g_action_map_add_action_entries(G_ACTION_MAP(app), app_actions, G_N_ELEMENTS(app_actions), wid);
